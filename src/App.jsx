...
import { AlertTriangle, CheckCircle2, Download, Filter, Gauge, Link2, Play, Search, ShieldAlert } from 'lucide-react';
import './styles.css';
...
function App() {
  ...
  const issueRows = useMemo(() => pages.flatMap((p) => (p.issues || []).map((i) => ({ url: p.url, score: p.score, statusCode: p.statusCode, title: p.title, ...i }))).sort((a, b) => SEVERITY_ORDER[a.severity] - SEVERITY_ORDER[b.severity]), [pages]);
    ...
    <div className="app">
      <section className="hero">
        <div className="hero-inner">
          ...
          <p>Crawl jullie website, prioriteer issues en exporteer duidelijke SEO-data. Nu met severity scores, duplicate checks, redirect chains, internal status checks, Open Graph, structured data validatie, accessibility checks en link health.</p>
        </div>
      </section>
